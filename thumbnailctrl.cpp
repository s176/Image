...
{
	int sz = (int)GetCount();
	//wxMessageBox(wxString::Format(wxT("%i"), sz));
	m_items.Add(item);
	m_firstSelection = -1;
	m_lastSelection = -1;
	m_focusItem = -1;
...
int wxThumbnailCtrlCmpFunc(wxThumbnailItem** item1, wxThumbnailItem** item2)
{
	...
	else if (sortMode == wxTHUMBNAIL_SORT_TYPE_UP || sortMode == wxTHUMBNAIL_SORT_TYPE_DOWN)
	{
		...
		wxFileName fname1(wxFileNameFromPath((*item2)->GetFilename()));
		//ext1 = fname1.GetExt();
		wxFileName fname2(wxFileNameFromPath((*item1)->GetFilename()));
		//ext2 = fname2.GetExt();
...
bool wxImageThumbnailItem::Load(wxThumbnailCtrl* ctrl, bool forceLoad)
{
	...
	if (wxFileExists(m_filename))
	{
		...
		int rcread;
		const char *zSql = "SELECT value FROM thumbs WHERE hash = ?";
		sqlite3_stmt *pStmt;
		rcread = sqlite3_prepare(m_db, zSql, -1, &pStmt, 0);
		sqlite3_bind_text(pStmt, 1, hash.c_str(), -1, SQLITE_STATIC);
		rcread = sqlite3_step(pStmt);
		wxImage image;
		if (rcread == SQLITE_ROW)
		{
			unsigned char* pzBlob;    /* Pointer to blob of data */
			int pnBlob;                      /* Length of data pointed to by zBlob */
			pnBlob = sqlite3_column_bytes(pStmt, 0);
			pzBlob = (unsigned char *)malloc(pnBlob);
			memcpy(pzBlob, sqlite3_column_blob(pStmt, 0), pnBlob);
			wxMemoryInputStream mis(pzBlob, pnBlob);
			image.LoadFile(mis);
		...
		{
			rcread = sqlite3_finalize(pStmt);
			const char *zSqlwrite = "INSERT INTO thumbs(hash, value) VALUES(?, ?)";
			sqlite3_stmt *pStmtwrite;
			int rcwrite;
			rcwrite = sqlite3_prepare(m_db, zSqlwrite, -1, &pStmtwrite, 0);

			if (isRaw)
			{
				LibRaw iProcessor;
				iProcessor.open_file(m_filename.char_str());
				iProcessor.unpack_thumb();
				wxMemoryInputStream inputstream(iProcessor.dcraw_make_mem_thumb()->data, iProcessor.dcraw_make_mem_thumb()->data_size);
				iProcessor.dcraw_make_mem_thumb();
				image.LoadFile(inputstream, wxBITMAP_TYPE_JPEG);
				...
				image.SaveFile(outstream, wxBITMAP_TYPE_JPEG);
				wxStreamBuffer* theBuffer = outstream.GetOutputStreamBuffer();
				sqlite3_bind_text(pStmtwrite, 1, hash.c_str(), -1, SQLITE_STATIC);
				sqlite3_bind_blob(pStmtwrite, 2, theBuffer->GetBufferStart(), theBuffer->GetBufferSize(), SQLITE_STATIC);
				rcwrite = sqlite3_step(pStmtwrite);
				rcwrite = sqlite3_finalize(pStmtwrite);
			}
			else 
			{
					...
					{
						wxMemoryInputStream mis(exif.m_exifinfo->ThumbnailPointer, exif.m_exifinfo->ThumbnailSize);
						image.LoadFile(mis);
						fclose(pFile);		
						if (!image.IsOk())
						{
							image.LoadFile(m_filename, wxBITMAP_TYPE_ANY);
						}
					}
					else
					{
